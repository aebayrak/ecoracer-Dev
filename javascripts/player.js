...
class LocalData {
    ...
    // logged data
    acc_keys = []; // at what postions did the user press/release accelerator
    brake_keys = []; // at what positions did the user press/release brake

    // the next three variables are snapshot at the same distance interval as a triplet
    // for plotting at end of game.
    save_x = []; // saved x position of current play
    save_v = []; // saved velocity at save_x position
    save_eff = []; // saved efficiency at save_x position
};
...
class Player {
    ...
    UpdateVariables() {
        ...
        this.localData.vehSpeed = Math.round(
        ...
        this.localData.battstatus =
            Math.round(1000 - (this.localData.consumption / 3600 / 1000 / max_batt) * 1000) /
            10;
    ...
    UpdateUI() {
        if (this.firendlyName === Player.NAMES.HUMAN) {
            ...
            $("#effval").html( "Motor Efficiency: " + displayVaule + "%" );
        }
    }
};
